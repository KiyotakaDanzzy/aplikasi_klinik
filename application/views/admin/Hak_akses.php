<script type="text/javascript">
    ...
    function get_data() {
        ...
        $.ajax({
            url: '<?php echo base_url(); ?>admin/hak_akses/result_data',
            data: {
                cari
            },
            type: "POST",
            ...
            success: function(res) {
                ...
                if (res.result) {
                    ...
                    for (const item of res.data) {
                        let namaGrup = item.nama_grup_hak_akses ? item.nama_grup_hak_akses : '-';
                        table += `
                          <tr>
                              <td>${i}</td>
                              <td>${item.nama_hak_akses}</td>
                              <td>${item.link}</td>
                              <td>${namaGrup}</td>
                              <td>
                                  <div class="text-center">
                                      <a href="<?php echo base_url(); ?>admin/hak_akses/view_edit/${item.id}"><button type="button" class="btn btn-shadow btn-sm btn-info"><i class="fas fa-pencil-alt"></i></button></a>
    ...
    function hapus(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '<?php echo base_url(); ?>admin/hak_akses/hapus',
                    method: 'POST',
                    data: {
                        id
                    },
        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header d-flex flex-wrap gap-2 justify-content-between align-items-center pt-3 pb-3">
                    ...
                    <a href="<?php echo base_url(); ?>admin/hak_akses/view_tambah">
                    ...
                    <div class="row mb-3">
                        <div class="col-sm-3">
                            <div class="input-group">
                                ...
                                <input type="text" class="form-control" id="cari" placeholder="Cari Nama Hak Akses">
                            </div>
                        </div>
                    </div>
