...
<script type="text/javascript">
  ...
  function get_data() {
      ...
      $.ajax({
          url : '<?php echo base_url();?>poli/pol_gigi/result_data',
          data : {cari},
          type : "POST",
          dataType :  "json",
          beforeSend : () => {
                          ...
                          <tr>
                              ...
                              <td>
                                  <div class="text-center">
                                      <a href="<?php echo base_url(); ?>poli/pol_gigi/view_edit/${item.id}"><button type="button" class="btn btn-shadow btn-sm btn-info"><i class="fas fa-pencil-alt"></i></button></a>
  ...
  function paging($selector){
      ...
      // Pastikan Anda telah menyertakan library Pagination.js
      // Jika library tidak ditemukan, bagian ini tidak akan berfungsi.
      if (typeof Pagination !== 'undefined') {
        ...
        // Fallback jika Pagination.js tidak ada
        var items = $("#table-data tbody tr");
        var numItems = items.length;
        var perPage = parseInt(jumlah_tampil);
        items.slice(perPage).hide();
        if(numItems > perPage){
            $('#pagination').html('<small class="text-danger">Library Pagination.js tidak ditemukan.</small>');
        }
      }
  }
  ...
  function hapus(id) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          $.ajax({
              url : '<?php echo base_url(); ?>poli/pol_gigi/hapus',
              method : 'POST',
              data : {id},
              dataType : 'json',
              success: function (res){
        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header d-flex flex-wrap gap-2 justify-content-between align-items-center pt-3 pb-3">
                    ...
                    <a href="<?php echo base_url(); ?>poli/pol_gigi/view_tambah"><button type="button" class="btn btn-success"><i class="fas fa-plus"></i> Tambah</button></a>
                </div>
                <div class="card-body">
                    <div class="row mb-3">
                        <div class="col-sm-3">
