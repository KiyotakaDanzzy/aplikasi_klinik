<script type="text/javascript">
    ...
    function searchDiagnosa(cari = 'Poli Gigi') {
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.length > 0) {
                    response.forEach((item, i) => {
                        rows += `<tr style="cursor:pointer;" onclick="addDiagnosa(${item.id}, '${item.nama_diagnosa.replace(/'/g, "\\'")}')"><td>${i+1}</td><td>${item.nama_diagnosa}</td></tr>`;
                    });
                } else {
    ...
    function searchTindakan(cari = 'Poli Gigi') {
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.length > 0) {
                    response.forEach((item, i) => {
                        ...
                        rows += `<tr style="cursor:pointer;" onclick="addTindakan(${item.id}, '${item.nama.replace(/'/g, "\\'")}', ${item.harga_raw})">
                                    <td>${i+1}</td>
                                    <td>${item.nama}</td>
                                    <td>${hargaDisplay}</td>
                                </tr>`;
            ...
            success: function(res) {
                if (res.status) {
                    Swal.fire({
                        ...
                        window.location.href = '<?php echo base_url('antrian/panel_dokter'); ?>';
                    });
                } else {
                    Swal.fire({
                        title: 'Gagal!',
